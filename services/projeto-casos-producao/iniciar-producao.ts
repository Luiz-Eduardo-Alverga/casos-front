...
/** Estrutura de erro retornada pela API (400 ou 422). O tipo de falha vem de `caso_aberto`. */
 ...
 * O `code` é definido pelo corpo: `caso_aberto` preenchido = caso já aberto; `null` = tempo estimado/não planejado.
 */
export class IniciarProducaoError extends Error {
  ...
  constructor(payload: IniciarProducaoErrorPayload, status: 400 | 422) {
    ...
    this.code =
      payload.caso_aberto != null
        ? "CASO_JA_ABERTO"
        : "TEMPO_ESTIMADO_OBRIGATORIO";
  }
}
 ...
 *
 * @throws {IniciarProducaoError} Em 400 ou 422; use `code`/`caso_aberto` no corpo para distinguir o motivo
  ...
  if (!response.ok) {
    if (response.status === 400 || response.status === 422) {
      const payload: IniciarProducaoErrorPayload = {
        success: false,
        message: body?.message ?? "Erro ao iniciar produção",
        caso_aberto: body?.caso_aberto ?? null,
      };
      throw new IniciarProducaoError(payload, response.status as 400 | 422);
    }
    throw new Error(body?.error ?? body?.message ?? "Erro ao iniciar produção");
  }
