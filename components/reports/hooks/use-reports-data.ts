...
export function useReportsData(filtros: ReportsFiltrosAplicados) {
  const { data: setores, isSuccess: setoresReady, isLoading: isLoadingSetores } =
    useSetores();
  const setorNome = resolveSetorNome(filtros.setor, setores);
  const hasSetor = Boolean(setorNome);

  // Enquanto o catálogo de setores carrega (ou o setor do usuário ainda vai
  // ser aplicado na URL), a query de projeto-memoria fica desabilitada.
  // Tratar isso como loading evita o flash do empty state "Selecione um setor".
  const willAutoApplySetor =
    !filtros.setor?.trim() &&
    setoresReady &&
    Boolean(resolveSetorIdByNome(getUser()?.setor, setores));
  const isBootstrappingSetor =
    !hasSetor && (isLoadingSetores || !setoresReady || willAutoApplySetor);
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
    isLoading: isLoadingMemoria,
  } = useProjetoMemoria(params, {
    enabled: hasSetor,
    refetchInterval: hasSetor ? AUTO_REFETCH_INTERVAL_MS : false,
  });

  const isLoading = isBootstrappingSetor || (hasSetor && isLoadingMemoria);
