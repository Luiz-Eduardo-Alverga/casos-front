...
export const ESTIMADO_FILTRO_VALUES = ESTIMADO_FILTRO_OPTIONS.map(
  (opt) => opt.value,
) as [
  (typeof ESTIMADO_FILTRO_OPTIONS)[number]["value"],
  ...(typeof ESTIMADO_FILTRO_OPTIONS)[number]["value"][],
];
...
export const casosFiltrosParsers = {
  ...
  estimado_filtro: parseAsStringLiteral(ESTIMADO_FILTRO_VALUES).withDefault(
    "todos",
  ),
...
export type CasosFiltrosNuqsState = {
  ...
  estimado_filtro: (typeof ESTIMADO_FILTRO_VALUES)[number];
  status_id: string[];
