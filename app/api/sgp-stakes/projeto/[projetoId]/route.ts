...
export async function GET(
  ...
  return withPermission("list-project", async () => {
    try {
      ...
      const response = await api.get(`/sgp-stakes/projeto/${projetoId}`, {
        params: {
          ...(per_page ? { per_page } : {}),
      ...
      console.error("Erro na API Route de sgp-stakes por projeto:", error);
      const { status, message } = extractApiError(
        error,
        "Erro ao buscar stakes do projeto",
      );
