...
export async function GET(
  ...
  return withPermission("list-project", async () => {
    try {
      ...
      const response = await api.get(`/sgp-riscos/projeto/${projetoId}`, {
        params: {
          ...(per_page ? { per_page } : {}),
      ...
      console.error("Erro na API Route de sgp-riscos por projeto:", error);
      const { status, message } = extractApiError(
        error,
        "Erro ao buscar riscos do projeto",
      );
