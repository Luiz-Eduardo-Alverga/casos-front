...
import type { CreateSgpCronogramaRequest } from "@/interfaces/sgp-cronograma";
...
function validateCronogramaBody(
  body: CreateSgpCronogramaRequest,
): Response | null {
...
export async function POST(request: Request) {
  return withPermission("edit-project", async () => {
    try {
      ...
      const body = (await request.json()) as CreateSgpCronogramaRequest;
      const validationError = validateCronogramaBody(body);
      if (validationError) return validationError;

      const response = await api.post("/sgp-cronograma", body, {
        headers: {
      ...
      console.error("Erro na API Route ao cadastrar sgp-cronograma:", error);
      const { status, message } = extractApiError(
        error,
        "Erro ao cadastrar tarefa do cronograma",
      );
