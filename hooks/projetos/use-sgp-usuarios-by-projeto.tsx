"use client";

import { useInfiniteQuery, useQuery } from "@tanstack/react-query";
import {
  getSgpUsuariosByProjeto,
  type GetSgpUsuariosByProjetoParams,
} from "@/services/sgp-usuarios/get-sgp-usuarios-by-projeto";

export interface UseSgpUsuariosByProjetoOptions {
  enabled?: boolean;
  per_page?: number;
  cursor?: string | null;
}

export function useSgpUsuariosByProjeto(
  projetoId: number | string | null | undefined,
  options?: UseSgpUsuariosByProjetoOptions,
) {
  const enabled = options?.enabled ?? true;
  const shouldFetch = projetoId != null && projetoId !== "" && enabled;

  const params: GetSgpUsuariosByProjetoParams | null = shouldFetch
    ? {
        projetoId: projetoId as number | string,
        per_page: options?.per_page,
        cursor: options?.cursor,
      }
    : null;

  return useQuery({
    queryKey: [
      "sgp-usuarios",
      "projeto",
      projetoId ?? "",
      options?.per_page ?? "",
      options?.cursor ?? "",
    ],
    queryFn: () => getSgpUsuariosByProjeto(params!),
    enabled: Boolean(params),
  });
}

export function useSgpUsuariosByProjetoInfinite(
  projetoId: number | string | null | undefined,
  options?: UseSgpUsuariosByProjetoOptions,
) {
  const enabled = options?.enabled ?? true;
  const shouldFetch = projetoId != null && projetoId !== "" && enabled;
  const perPage = options?.per_page ?? 15;

  return useInfiniteQuery({
    queryKey: [
      "sgp-usuarios",
      "projeto",
      "infinite",
      projetoId ?? "",
      perPage,
    ],
    queryFn: ({ pageParam }) =>
      getSgpUsuariosByProjeto({
        projetoId: projetoId as number | string,
        per_page: perPage,
        cursor: pageParam ?? null,
      }),
    initialPageParam: null as string | null,
    getNextPageParam: (lastPage) =>
      lastPage.pagination.has_more
        ? lastPage.pagination.next_cursor
        : undefined,
    enabled: shouldFetch,
  });
}
